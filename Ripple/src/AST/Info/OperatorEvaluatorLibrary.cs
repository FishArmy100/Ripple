    ...
    class OperatorEvaluatorLibrary
    {
        ...
        public readonly OperatorEvaluator<ValueInfo, List<ValueInfo>> Calls =
            new OperatorEvaluator<ValueInfo, List<ValueInfo>>(
                (o, p, t) => new ASTInfoError("Too many call operators for type '" + o.Type + "' with arguments: '" + p.ConvertAll(v => v.ToString()).Concat("', '") + "'.", t),
                (o, p, t) => new ASTInfoError("No call operators for type '" + o.Type + "' with arguments: '" + p.ConvertAll(v => v.ToString()).Concat("', '") + "'.", t));

        public readonly OperatorEvaluator<ValueInfo, ValueInfo> Indexers =
            new OperatorEvaluator<ValueInfo, ValueInfo>(
                (indexed, arg, tok) => new ASTInfoError("Multiple index operators possible for type '" + indexed.Type + "' with argument '" + arg + "'.", tok),
                (indexed, arg, tok) => new ASTInfoError("No index operator for type '" + indexed.Type + "' with argument '" + arg + "'.", tok));

        public readonly OperatorEvaluator<TypeInfo, ValueInfo> Casts =
            new OperatorEvaluator<TypeInfo, ValueInfo>(
                (type, value, tok) => new ASTInfoError("Multiple possible cast operators from type '" + value.Type + "', to type '" + type + "'.", tok),
                (type, value, tok) => new ASTInfoError("No cast operator from type '" + value.Type + "', to type '" + type + "'.", tok));
    }
}
